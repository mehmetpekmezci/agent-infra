...
then
    ...
    #
    docker run -d --name  $DOCKER_NAME -p 6379:6379 -v $AGENT_INFRA_DATA_DIR/redis_data:/data redis redis-server --appendonly yes --requirepass "redis_password" >& $AGENT_INFRA_LOG_DIR/redis.log &
fi
