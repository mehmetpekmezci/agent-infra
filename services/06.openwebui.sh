...
then
    ...
    sudo docker run -d -p 8080:8080 --add-host=host.docker.internal:host-gateway -e OPENAI_API_BASE_URL=http://host.docker.internal:3000/v1 -v $AGENT_INFRA_DATA_DIR/open_web_ui_data:/app/backend/data --name $DOCKER_NAME  ghcr.io/open-webui/open-webui:main
